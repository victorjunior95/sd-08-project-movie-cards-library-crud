...
describe('6 - Insira um link na página inicial para `NewMovie` para criar novos cartões', () => {
  skip.it('a página inicial deverá conter um link "ADICIONAR CARTÃO". Esse link deve redirecionar para a página de criação de filmes', async () => {
  ...
  skip.it('"NewMovie" deverá conter um formulário que faz uma requisição para API para criar um novo filme. Após a criação, a pessoa usuária deverá ser redirecionada para a página inicial', async () => {
    await cleanup();
...
describe('7 - Adicione um link para deletar um cartão em `MovieDetails`', () => {
  skip.it('"MovieDetails" deverá conter um botão com o texto "DELETAR"', async () => {
    for (const movie of readMovies()) {
      const { container, unmount, findByText } = renderPath('/movies/' + movie.id);
      await waitFor(() => movieAPI.getMovie(movie.id));
  ...
  skip.it('o botão "DELETAR" deverá fazer uma requisição para a API para excluir o filme em questão', async () => {
    resetStorage();
