...
describe('2 - Faça uma requisição para buscar e mostrar a lista de filmes quando `MovieList` for montado', () => {
  xtest('deverá exibir o texto "Carregando..." enquanto estiver fazendo a requisição', async () => {
    const { container, unmount, getByText } = renderPath('/');
    expect(getByText('Carregando...'));
    await waitFor(() => movieAPI.getMovies())
    unmount();
  })
  xtest('deverá exibir um MovieCard para cada filme retornado pela API', async () => {
...
describe('3 - Insira um link para a página de detalhes de um filme dentro de `MovieCard`', () => {
  xtest('deve exibir pelo menos o título e a sinopse de seu respectivo filme', async () => {
  ...
  xtest('deve conter um link com o texto "VER DETALHES" que redireciona para a página de detalhes do filme', async () => {
    const { unmount, getAllByText } = renderPath('/');
    await waitFor(() => movieAPI.getMovies());
...
describe('4 - Faça uma requisição para buscar o filme que deverá ser renderizado dentro de `Movie Details`', () => {
  ...
  xit('deverá exibir o texto "Carregando..." enquanto estiver fazendo a requisição', async () => {
    for (const movie of readMovies()) {
      await cleanup();
  ...
  xit('deverá exibir o título, o subtítulo, a sinopse, a imagem e o gênero do filme', async () => {
    for (const movie of readMovies()) {
  ...
  xit('deverá conter um botão com o texto "VOLTAR" que redireciona para a página inicial', async () => {
    for (const movie of readMovies()) {
  ...
  xit('deverá conter um botão com o texto "EDITAR" que redireciona para a página de edição de filme', async () => {
    for (const movie of readMovies()) {
      const { container, unmount, findByText } = renderPath('/movies/' + movie.id);
      await waitFor(() => movieAPI.getMovie(movie.id));
...
describe('5 - Realize uma requisição para buscar o filme que será editado em `EditMovie`', () => {
  ...
  xit('deverá conter um formulário preenchido com o título, subtítulo, sinopse, caminho da imagem e gênero do filme selecionado', async () => {
    for (const movie of readMovies()) {
  ...
  xit('Quando clicar no botão de submit, deverá fazer uma requisição para API para atualizar o filme selecionado. Após a conclusão da atualização a pessoa usuária deverá ser redirecionada para a página inicial', async () => {
    for (const movie of readMovies()) {
      await cleanup();
...
describe('6 - Insira um link na página inicial para `NewMovie` para criar novos cartões', () => {
  xit('a página inicial deverá conter um link "ADICIONAR CARTÃO". Esse link deve redirecionar para a página de criação de filmes', async () => {
  ...
  xit('"NewMovie" deverá conter um formulário que faz uma requisição para API para criar um novo filme. Após a criação, a pessoa usuária deverá ser redirecionada para a página inicial', async () => {
    await cleanup();
...
describe('7 - Adicione um link para deletar um cartão em `MovieDetails`', () => {
  xit('"MovieDetails" deverá conter um botão com o texto "DELETAR"', async () => {
    for (const movie of readMovies()) {
      const { container, unmount, findByText } = renderPath('/movies/' + movie.id);
      await waitFor(() => movieAPI.getMovie(movie.id));
  ...
  xit('o botão "DELETAR" deverá fazer uma requisição para a API para excluir o filme em questão', async () => {
    resetStorage();
